...
plt.title("Training Reward Over Episodes (Simplified Q-table)")
plt.grid(True)
plt.tight_layout()
plt.show()
...
accuracies = [evaluate_action_accuracy(return_only=True, steps=200) for _ in range(10)]
avg = sum(accuracies) / len(accuracies)
stddev = (sum((x - avg) ** 2 for x in accuracies) / len(accuracies))**0.5
print(f"Average: {avg:.2f}%, StdDev: {stddev:.2f}%")
